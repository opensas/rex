...
public class VariableController extends Controller {
	...
	public static void doRead(Map<String, String> variable, 
		...
		List<Variable> variables = Variable.all().fetch(pagina, paginado);
	...
	public static void createTestData() {
		...
		for(int i = 1; i<=1000; i++) {
			v = new Variable();
			v.codigo = "VARIABLE_" + String.valueOf(i);
			v.contenido = "Contenido de " + v.codigo;
			v.descripcion = "Descripción de " + v.codigo;
			v.save();
			v.codigo = v.codigo + " (" + v.id.toString() + ")";
			v.save();
		}
		doRead(null, "", 0L, 1, lib.TemplateHelper.DEFAULT_PAGE_SIZE);
	}
